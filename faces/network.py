...
def get_ave_xy(hmi, n_points = 4, thresh=0, input_width=96, input_height=96):
    """
...
def get_points(imgarr: np.ndarray,
               output_height: int = 96,
               output_width=96,
               nClasses=15,
               n_points_final=-1):
    """
    Receives image and returns 2 lists with x and y coordinates
    """

    imgarr = imgarr.reshape(1, 96, 96, 1)  # reshape to specific 4-dim tensor
    y_pred_test = model.predict(imgarr)

    # some magic from article
    y_pred_test = y_pred_test.reshape(-1, output_height, output_width, nClasses)
    y_pred_test_xy = transfer_target(y_pred_test, thresh=0, n_points=n_points_final)

    list_x, list_y = [], []
    for i in range(len(y_pred_test_xy[0])):
        if i % 2 == 0:
            list_x.append(y_pred_test_xy[0][i])
        else:
            list_y.append(y_pred_test_xy[0][i])
